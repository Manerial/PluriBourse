...
Construit sur des technologies web standard et éprouvées, déployé sur votre propre serveur via un environnement isolé et reproductible, PluriBourse est conçu pour s'adapter à toute association organisant n'importe quel type de vente d'occasion, plusieurs fois par an si nécessaire.
...
Le guide d'installation est aussi important que le produit lui-même — c'est lui qui transforme le logiciel en un outil que le personnel d'une association peut déployer sans assistance un samedi après-midi.
...
- Des configurations rigides, spécifiques à une machine, qui cassent le logiciel au moindre changement d'infrastructure.
...
- Des architectures et technologies datées, installées directement sur le serveur sans suivi des modifications, rendant l'investigation fastidieuse.
...
**Une architecture reproductible, sans dépendance à la machine.** Le déploiement est décrit dans un fichier de configuration, pas dans la tête de quelqu'un. Changer de serveur ou reconstruire après une panne ne nécessite aucune connaissance préalable de la configuration. Pas de paramètre implicite hérité d'une installation précédente.

**Des technologies standard, lisibles par n'importe quel développeur.** Les outils choisis sont des standards largement documentés et répandus. Une architecture sans détours et un historique versionné de toutes les évolutions de la base de données font que tout développeur familier de ces technologies peut ouvrir le code, comprendre ce qu'il fait, et diagnostiquer un problème sans l'auteur original.

**Un guide d'installation conçu comme un livrable à part entière.** Si une personne non technique peut installer la plateforme seul un samedi après-midi, il peut aussi suivre un runbook de diagnostic le jour d'une panne. La documentation n'est pas une annexe : c'est ce qui transforme un logiciel fonctionnel en un outil qu'une association peut posséder et maintenir de façon autonome.

## Aperçu

PluriBourse structure chaque événement en cinq phases contrôlées par l'administrateur :

**1. Phase de préparation** — L'administrateur configure l'édition : taux de commission, langue des documents, catégories d'articles et mapping (assignation) catégorie-table. Les catégories et le mapping d'une édition précédente peuvent être copiés en un clic.

**2. Phase de dépôt** — Les bénévoles inscrivent les vendeurs (création de nouveaux profils ou récupération d'existants par nom ou email) et cataloguent chaque article avec un nom, un prix, une catégorie et un indicateur complet/incomplet. La table est assignée automatiquement selon la catégorie. L'application génère des codes-barres, imprime des étiquettes sur rouleau thermique adhésif 57mm et produit un bordereau de dépôt par vendeur. Les lots (ensembles indivisibles à prix global) sont supportés par l'application.
...
- Génération de codes-barres + impression d'étiquettes sur rouleau thermique adhésif 57mm
...
- Interface caisse avec support scanner USB à code-barres (compatible toutes dispositions de clavier)
- Gestion des lots au scanner : validation bloquée tant que le lot est incomplet
- Moyen de paiement enregistré à chaque transaction (espèces, chèque, carte)
- En cas de paiement en espèces, calcul automatique de la monnaie à rendre si le montant remis est saisi
- Impression de factures acheteur

*Post-vente*

- Bilan de vente imprimable par vendeur (articles vendus, invendus avec table, solde net)
- Bouton « Solder » : enregistre le reversement remis au vendeur
