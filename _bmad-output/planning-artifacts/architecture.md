...
- **F2 — Gestion des Éditions (12 EFs) :** Cycle de vie des phases contrôlé par l'administrateur (Préparation → Dépôt → Vente → Post-vente → Clôturé). Une seule édition active à la fois. Retour arrière de phase supporté. Action optionnelle destructrice « Archiver l'Édition ».
...
| Déclencheur (bis) | La **déconnexion explicite** d'un caissier annule son panier actif et libère ses réservations de lot (FR-110), **sans** émettre `basket-cancelled` — la session invalidée déconnecte d'elle-même les autres onglets du même utilisateur | Réutilise la routine d'annulation de panier partagée (`BasketCancellationService`) ; **pas** de broadcast (`SseEmitterRegistry.broadcast` n'est pas ciblé → il annulerait le panier des autres caissiers) *(SCP 2026-09-03 ; clause SSE corrigée par la Story 4.8)* |
| Déclencheur (ter) | Le **balayage de postes inactifs** (`@Scheduled`) annule un panier actif dont le heartbeat s'est tu au-delà du seuil et libère ses réservations de lot (FR-110 / Story 4.9) | Réutilise `BasketCancellationService.cancelBasketSilently` — **aucun** broadcast `basket-cancelled` (même raison que la déconnexion : diffusion non ciblée) ; le poste concerné a de toute façon cessé de communiquer *(SCP 2026-09-04)* |
