...
| Déclencheur (bis) | La **déconnexion explicite** d'un caissier annule son panier actif et émet `basket-cancelled` vers ses autres onglets (FR-110) | Réutilise le même événement et le même `SseEmitterRegistry` que la transition de phase ; la routine d'annulation de panier est partagée *(SCP 2026-09-03)* |
