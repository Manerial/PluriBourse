...
- Modals pour confirmer des actions non destructives (préférer l'inline ou le toast)
...
| **Dialog de confirmation** | Transitions de phase, actions destructives, archivage d'édition | Bloque l'interaction derrière un overlay. Toujours : titre + conséquence + bouton confirm + bouton annuler. Focus piégé dans le dialog (accessibilité). Focus initial sur le bouton d'annulation (action sûre). À la fermeture (quelle que soit la cause — bouton Annuler, Échap, ou confirmation), rétablir le focus sur l'élément déclencheur du dialog. Fermeture : bouton annuler ou Echap. |
...
| **Page file d'impression** | Admin — `/admin/print-queue` · toutes phases (FR-079) | Vue organisée par imprimante enregistrée. Chaque imprimante est présentée dans une carte : nom · type (thermique / A4) · statut de connexion (chip vert « Connectée » / rouge « Hors ligne ») · profondeur de file · job en cours · dernière erreur. Si une imprimante était inaccessible au démarrage du serveur, la carte affiche un bandeau d'alerte : « Imprimante inaccessible au démarrage — vérifiez la connexion Bluetooth / réseau. » Liste des jobs par imprimante : colonnes type de document · vendeur ou article concerné · statut (En attente / En cours / Imprimé / Erreur). Actions par job : "Relancer" · "Ignorer" (disponible si statut Erreur). Mise à jour en temps réel via SSE (`print-job-updated`). État vide par imprimante : « Aucun travail en file. » |
| **Gestion des imprimantes (Admin)** | Admin — `/admin/printers` · toutes phases (FR-076, FR-077) | Deux sections : **Imprimantes thermiques** et **Imprimantes A4**. Chaque section liste les imprimantes enregistrées avec : nom · statut de connexion · largeur (thermiques uniquement). Bouton "Ajouter une imprimante thermique" / "Ajouter une imprimante A4" (primary) en haut de chaque section. **Formulaire thermique :** nom d'affichage (texte) · port série (liste déroulante des ports série disponibles sur le serveur — affiche le nom descriptif de l'appareil Bluetooth appairé) · largeur (sélecteur 57 mm / 80 mm). **Formulaire A4 :** nom d'affichage (texte) · adresse IP ou hostname (texte) · port TCP (numérique, défaut 9100). À la sauvegarde : toast succès "Imprimante enregistrée." et la file correspondante est instanciée. Suppression d'imprimante : bouton icône `delete` par ligne, dialog de confirmation "Supprimer [nom] ? Les travaux en cours seront perdus." |
