...
17. **Découvrabilité.** Une entrée « Liste des ventes » est ajoutée à la sidebar bénévole, visible en phase Vente (même conditionnement que l'entrée « Caisse »), pointant vers `/volunteer/sales`.
    **Corrigé le 2026-09-02** — une entrée équivalente est aussi ajoutée à la **sidebar admin** (section « Édition active », `@if phase === 'SALE'`, icône `receipt_long`, libellé `nav.admin.sales`, pointant vers `/volunteer/sales`). Motif : télécharger/ouvrir/lire le rapport agrégé pour consulter les ventes est trop lourd à l'usage ; l'endpoint `GET /pos/sales` est déjà ouvert à ADMIN et `salePhaseGuard` ne filtre pas par rôle, donc aucun changement backend ni de route.
...
`/volunteer/sales` sous `volunteerRoutes`, `canActivate: [salePhaseGuard]` — miroir exact de `/volunteer/pos`. `SalesListComponent` dans `features/volunteer/sales/`. La route `/volunteer` n'a pas de garde de rôle et `salePhaseGuard` ne vérifie que la phase, donc l'admin peut l'atteindre ; l'endpoint `GET /pos/sales` est ouvert aux deux rôles.

**Confirmé avec l'utilisateur (2026-09-01) :** on part sur une configuration minimale (Vente uniquement, entrée sidebar bénévole en phase Vente, pas d'accès Post-vente, pas d'entrée admin) — « on verra à l'usage ».

**Corrigé le 2026-09-02 (revirement utilisateur) :** l'entrée sidebar **admin** est finalement ajoutée (section « Édition active », `@if phase === 'SALE'`, → `/volunteer/sales`). L'utilisateur revient sur le « pas d'entrée admin » : passer par le rapport agrégé (télécharger, ouvrir, lire) est trop pénible au quotidien. Changement front only (une entrée `<a>` + clé i18n `nav.admin.sales` fr/en + un test de nav) — backend, route et guard inchangés. Post-vente reste hors périmètre.
...
2. **Accès de l'écran.** ✅ **Résolu (2026-09-01), révisé (2026-09-02).** `/volunteer/sales`, `salePhaseGuard`, entrée sidebar bénévole en phase Vente. Pas d'accès Post-vente. ~~Pas d'entrée admin~~ → **entrée admin ajoutée le 2026-09-02** (sidebar admin, section « Édition active », phase Vente, → `/volunteer/sales`) : revirement utilisateur, le rapport agrégé est trop lourd pour une simple consultation des ventes.
...
- `app-layout.component.html` : entrée sidebar bénévole `/volunteer/sales` (icône `receipt_long`, `@if phase === 'SALE'`, sous `/volunteer/pos`). `app-layout.component.ts` : `/volunteer/sales` ajouté à `PHASE_BOUND_VOLUNTEER_PATHS` (décision de la story). ~~Pas d'entrée sidebar admin~~ → **entrée sidebar admin ajoutée le 2026-09-02** (section « Édition active », `@if phase === 'SALE'`, icône `receipt_long`, `nav.admin.sales`, → `/volunteer/sales`) ; voir Change Log 2026-09-02.
...
- 2026-09-02 — revirement utilisateur sur la Q2 (« pas d'entrée admin » confirmée hier, corrigée aujourd'hui). Ajout d'une entrée sidebar **admin** « Liste des ventes » : `app-layout.component.html` section « Édition active », `@if (currentEdition()?.phase === 'SALE')`, icône `receipt_long`, libellé `nav.admin.sales`, `routerLink="/volunteer/sales"` (pas de route alias `/admin/sales` — la route bénévole + `salePhaseGuard` suffisent, aucun filtre de rôle). Clé i18n `nav.admin.sales` ajoutée fr (« Liste des ventes ») + en (« Sales list »). Test `app-layout.component.spec.ts` : « shows the sales list link only in the Sale phase ». Aucun changement backend, route ou guard. AC 17, section « Route et guard », Q2 et Completion Notes mis à jour en place. Suite frontend verte.
