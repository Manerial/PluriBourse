...
### Superseded by Story 2.11 (SCP 2026-09-07)

`SseService.onerror` ne déconnecte plus sur `CLOSED` de transport (Story 2.11, SCP 2026-09-07) ; l'AC4 est désormais tenue aussi pour un échec *dur*, via une reconnexion applicative + une sonde `GET /api/auth/me` qui délègue le logout à `authInterceptor`. Le commentaire `onerror` de la 2.6 supposant « CLOSED ⇒ session perdue » est superseded — cette hypothèse ne tenait que tant que `SseEmitterRegistry` fermait chaque émetteur après chaque broadcast (changé par la Story 4.8, qui garde les émetteurs ouverts). Le `currentEdition.set(null)` ajouté ici par le patch de 2e passe est également retiré de `SseService` (remis à `null` au bon moment par le flux de login).
