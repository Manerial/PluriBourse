...
### Note miroir — Story 2.11 (SCP 2026-09-07)

Garder les émetteurs SSE ouverts entre deux évènements (changement de cette story sur `SseEmitterRegistry`, cf. « autres onglets → 401 → `/login` » ci-dessus) est la raison pour laquelle un `CLOSED` permanent ne peut plus être assimilé à « session expirée » côté `SseService` : avec une session valide, un simple timeout de reverse-proxy produit ce `CLOSED`. Le **keepalive planifié** ajouté par la Story 2.11 (`SseEmitterRegistry.sendKeepalive`, `@Scheduled` ~20 s) est la contrepartie de ce choix — il empêche un intermédiaire de couper une connexion SSE oisive. La branche `onerror` → logout de la Story 2.6 est retirée en conséquence (reconnexion applicative + sonde `GET /api/auth/me`).
