...
#
# --start : redémarrage rapide sur une install déjà en place (containers + configuration réseau +
# service PrinterBridge), sans repasser par les vérifications de prérequis/dépôt/paquets — utile
# en particulier pour PrinterBridge, qui ne démarre jamais tout seul au boot par choix (cf. CLAUDE.md
# de PrinterBridge) contrairement aux containers Docker (restart: unless-stopped). Pas un substitut à
...
for arg in "$@"; do
    case "$arg" in
        ...
        --start)
            ...
            ;;
        *)
            echo "Argument inconnu : $arg (--update et --start sont supportés)." >&2
            exit 1
            ;;
    esac
done

# --start seul saute les étapes d'installation (prérequis, dépôt, Docker Engine, paquet
# PrinterBridge) pour n'exécuter que le démarrage — --update prime dessus s'il est aussi présent
# (rien à sauter, on veut justement tout revérifier dans ce cas).
SKIP_INSTALL_STEPS=false
if [[ "${START_MODE}" == "true" && "${UPDATE_MODE}" != "true" ]]; then
    SKIP_INSTALL_STEPS=true
fi
...
if [[ "${SKIP_INSTALL_STEPS}" != "true" ]]; then
    ...
    if [[ ! -d "${INSTALL_DIR}/.git" ]]; then
        echo "PluriBourse n'est pas encore installé dans ${INSTALL_DIR} — lance ce script sans --start d'abord." >&2
        exit 1
    fi
fi
...
# --start ne re-pull pas les images (une mise à jour d'images, ça reste le rôle de --update) — juste
...
if [[ "${SKIP_INSTALL_STEPS}" != "true" ]]; then
    ...
    if ! dpkg-query -W -f='${Version}' printerbridge >/dev/null 2>&1; then
        echo "PrinterBridge n'est pas encore installé — lance ce script sans --start d'abord." >&2
        exit 1
    fi
    log "--start : vérification de version PrinterBridge ignorée, service relancé tel quel."
fi
