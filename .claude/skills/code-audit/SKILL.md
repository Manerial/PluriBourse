---
name: code-audit
description: 'Audit complet du code PluriBourse (front Angular + back Spring Boot) via des agents parallèles indépendants et à scope restreint, un par axe (ou groupe d''axes partageant le même périmètre de fichiers) : qualité front, qualité back, opportunités de simplification (logique manuelle qui duplique une capacité déjà disponible ailleurs — relation JPA, méthode de repository, service partagé — et justesse des usages d''index/tri/bornes sur ces capacités), cohérence endpoints front/back, cohérence DTOs/modèles, code mort front, code mort back, clés i18n inutilisées/non traduites, CSS vs UX BMAD, sécurité & données perso dans les logs, calculs financiers BigDecimal, conformité des tests backend, conventions Angular strictes. Utiliser quand l''utilisateur dit "audit de code", "lance l''audit", "audite le code", "vérifie la qualité du code".'
---
...
| `front` | `sonnet` (défaut) | `pluribourse-frontend/src/app/**` uniquement (jamais `.angular/`, `dist/`, `node_modules/`) | 1. Qualité code front (nommage, duplication, typage, gestion d'erreurs, fuites RxJS/Signals) — 2. Code mort front (composants/services/routes/imports jamais utilisés) — 3. Conventions Angular obligatoires (standalone, Signals seuls, jamais de template inline, accolades systématiques) — 4. Opportunités de simplification front (voir consigne dédiée ci-dessous) | Clés i18n (`i18n`), CSS/UX (`css`) |
...
**Consigne dédiée « Opportunités de simplification » (section 4 des agents `back` et `front` uniquement) :**

> "En plus des sections précédentes, cherche spécifiquement des endroits où du code réimplémente manuellement (boucle, filtre, tri, comptage, recherche) quelque chose qu'une capacité déjà déclarée ailleurs dans le périmètre fournirait directement — une relation JPA (`@OneToMany`/`@ManyToOne` déjà présente sur l'entité concernée), une méthode de repository déjà existante avec la même sémantique, un service/util partagé, ou un signal/computed Angular déjà exposé côté front. Pour chaque candidat repéré, vérifie aussi la réciproque : si le code utilise déjà une telle capacité partagée, ses hypothèses (index 0-based vs 1-based, ordre de tri, bornes inclusives/exclusives, liste vide vs null) correspondent-elles bien à ce que la capacité fournit réellement ? Un usage incorrect d'une capacité par ailleurs correcte (ex: `List.indexOf()` — 0-based — utilisé pour afficher un rang humain 1-based) est un constat de cette section, pas juste un bug générique. Ne remonte que des cas concrets et vérifiés dans le code réel (capacité existante ET endroit qui pourrait s'en servir, ou usage existant ET incohérence constatée) — pas de suggestion architecturale spéculative sans les deux termes de la comparaison sous les yeux."
...
(fusionne la section 4 de `back` et la section 4 de `front` ici — un seul endroit pour ce thème transverse, même si les constats viennent de deux agents différents)
## Clés i18n
## CSS vs UX BMAD
...
- « Opportunités de simplification » (section 4 de `back`/`front`) est en lecture seule comme le reste du skill — elle liste des constats, elle n'applique jamais de correction elle-même (pour ça, voir le skill `/simplify`, qui lui modifie le code mais sur un diff, pas sur tout le repo). Volontairement fusionnée dans les agents `back`/`front` existants plutôt que d'être un axe séparé, pour ne pas payer une deuxième lecture des mêmes arbres de fichiers.
- Exemple réel qui a motivé cette section (repère de calibration qualitatif) : `Lot` a gagné une relation `@OneToMany` eager vers `Item` ; le code qui calculait la position d'un article dans son lot en filtrant/triant manuellement une liste passée en paramètre aurait dû être signalé comme "peut utiliser `lot.getItems()` directement" — et si un jour ce genre de remplacement est fait à moitié (ex: `List.indexOf()` utilisé pour un rang censé être 1-based), la section doit aussi le repérer comme incohérence d'usage, pas seulement comme simplification manquée.
